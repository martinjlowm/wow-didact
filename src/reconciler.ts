import { Region } from 'wow-classic-declarations';
...
function createPublicInstance(element: InternalElement, internalInstance: Instance) {
...
function instantiate(element: InternalElement, parentFrame: Region): Instance {
  ...
  if (typeof type === 'string') {
    ...
    const frame = createFrame(type, parentFrame, props as any);

    updateFrameProperties(frame, {} as any, props as any);

    const childElements = props.children || [];
    const childInstances = childElements.map((child) => instantiate(child, frame));

    const instance: Instance = { hostFrame: frame, element, childInstances, childInstance: null };
    return instance;
  } else {
    ...
    const updateProps: Partial<Instance> = { hostFrame, element, childInstance, publicInstance };
...
export function reconcile(
  ...
  if (!instance) {
    ...
    // Update host element
    updateFrameProperties(instance.hostFrame, instance.element.props as any, element.props as any);
...
export function render(element: InternalElement, container: Region) {
